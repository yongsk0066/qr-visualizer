  ...
  if (!dataReadingResult || !unmaskedMatrix || !dataModules) {
    return (
      <div className="step-column">
        <h2 className="font-medium mb-3">{t('4단계: 데이터 모듈 읽기', 'Step 4: Data Module Reading')}</h2>
        
        <div className="space-y-4">
          <p className="text-sm text-gray-600">
            {t('지그재그 패턴으로 데이터 모듈을 읽어 비트스트림을 생성합니다', 'Generate bit stream by reading data modules in zigzag pattern')}
          </p>
          
          <div className="p-8 bg-gray-50 rounded text-center">
            <div className="text-gray-400 text-3xl mb-2">📖</div>
            <div className="text-gray-500 text-sm">{t('QR 코드를 감지하면 데이터 읽기가 표시됩니다', 'Data reading will be displayed when QR code is detected')}</div>
          </div>
        </div>
      </div>
    );
    ...
    <div className="step-column">
      ...
      <div className="space-y-4">
        ...
        {!dataReadingResult || !unmaskedMatrix || !dataModules ? (
          <div className="p-8 bg-gray-50 rounded text-center">
            <div className="text-gray-400 text-3xl mb-2">📖</div>
            <div className="text-gray-500 text-sm">{t('QR 코드를 감지하면 데이터 읽기가 시작됩니다', 'Data reading will start when QR code is detected')}</div>
          </div>
        ) : (
          <>
            {/* 3개 컬럼 매트릭스 시각화 */}
        <div className="overflow-x-auto">
          <div className="flex gap-4 min-w-max justify-center">
            ...
            <div className="flex flex-col items-center">
              <div className="text-xs font-medium mb-2">{t('전체 매트릭스', 'Full Matrix')}</div>
              <Matrix
            ...
            {/* 데이터 영역만 표시 */}
            <div className="flex flex-col items-center">
              <div className="text-xs font-medium mb-2">{t('데이터 영역', 'Data Area')}</div>
              <Matrix
                matrix={unmaskedMatrix}
                size={unmaskedMatrix.length}
                scale={scale}
            ...
            {/* 지그재그 패턴 시각화 */}
            <div className="flex flex-col items-center">
              <div className="text-xs font-medium mb-2">{t('지그재그 읽기 패턴', 'Zigzag Reading Pattern')}</div>
              <Matrix
                matrix={unmaskedMatrix}
                size={unmaskedMatrix.length}
                scale={scale}
        ...
        <div className="p-3 bg-gray-50 rounded">
          <div className="text-xs font-medium mb-2">{t('비트스트림', 'Bit Stream')} ({totalBits} {t('비트', 'bits')})</div>
        ...
        <div className="p-3 bg-gray-50 rounded">
          <div className="flex justify-between items-center mb-2">
            <div className="text-xs font-medium">{t('전체 코드워드', 'All Codewords')}</div>
            <button
              ...
              title={t('코드워드를 클립보드에 복사', 'Copy codewords to clipboard')}
            >
              {isCopying ? t('✓ 복사됨', '✓ Copied') : t('📋 복사', '📋 Copy')}
            </button>
          </div>
          <div className="text-[10px] text-gray-600 mb-2">
            {t('총', 'Total')} {codewords.length}{t('개 = 데이터', ' = Data')} {dataCodewordCount}{t('개 + 에러 정정', ' + Error Correction')} {errorCorrectionCodewordCount}{t('개', '')}
          </div>
        ...
        <div className="p-3 bg-gray-50 rounded">
          ...
          <div className="flex items-center flex-wrap gap-2 text-xs">
            <div className="flex items-center">
              ...
              <span className="ml-1 text-gray-600">{dataCodewordCount}{t('개', '')}</span>
            </div>
            ...
            <div className="flex items-center">
              ...
              <span className="ml-1 text-gray-600">{errorCorrectionCodewordCount}{t('개', '')}</span>
            </div>
            <span className="text-gray-400 font-medium">=</span>
            <span className="bg-blue-100 px-2 py-0.5 rounded text-xs font-medium">
              {t('총', 'Total')} {codewords.length}{t('개', '')}
            </span>
          </div>
        </div>
        ...
        {/* 신뢰도 */}
        <div className="p-3 bg-gray-50 rounded">
          <div className="text-xs font-medium mb-2">{t('읽기 신뢰도', 'Reading Confidence')}</div>
          <div className="space-y-2">
            ...
            <div className="text-[10px] text-gray-500 leading-relaxed">
              {t('예상', 'Expected')} {codewords.length * 8}{t('비트 중', ' bits out of')} {totalBits}{t('비트를 읽음.', ' bits read.')}
              {confidence >= 1 
                ? t(" 모든 데이터를 성공적으로 읽었습니다.", " All data was read successfully.")
                : t(" 일부 데이터가 누락되어 에러 정정이 필요할 수 있습니다.", " Some data is missing and error correction may be needed.")
              }
            </div>
          </div>
        </div>
