      ...
      {binarization ? (
        <div className="space-y-4">
          <p className="text-sm text-gray-600">
            {t('binarization.description')}
          </p>

          {/* 이진화 결과 */}
          <div className="p-3 bg-gray-50 rounded">
            <div className="text-xs font-medium mb-2">{t('binarization.binarizationResult')}</div>
            <canvas 
              ref={binaryCanvasRef} 
          ...
          <div>
            ...
            >
              {showThreshold ? t('binarization.hideThresholdMap') : t('binarization.showThresholdMap')}
            </button>
            
            {showThreshold && (
              <div className="p-3 bg-gray-50 rounded">
                <div className="text-xs font-medium mb-2">{t('binarization.adaptiveThresholdMap')}</div>
                <canvas 
                ...
                />
                <div className="text-xs text-gray-500 mt-2">
                  {t('binarization.thresholdVisualization')}
                </div>
              </div>
            )}
          </div>
          ...
          {/* 파라미터 및 통계 */}
          <div className="p-3 bg-gray-50 rounded">
            <div className="text-xs font-medium mb-2">{t('binarization.binarizationInfo')}</div>
            <div className="grid grid-cols-2 gap-4">
              <div>
                <div className="text-[11px] text-gray-600 mb-1">{t('binarization.sauvolaParameters')}</div>
                <div className="space-y-1 text-xs">
                  <div className="flex justify-between">
                    <span className="text-gray-600">{t('binarization.windowSize')}:</span>
                    <span className="font-mono">{binarization.parameters.windowSize}px</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-gray-600">{t('binarization.kValue')}:</span>
                    <span className="font-mono">{binarization.parameters.k}</span>
                  </div>
                </div>
              </div>
              <div>
                <div className="text-[11px] text-gray-600 mb-1">{t('binarization.pixelStatistics')}</div>
                <div className="space-y-1 text-xs">
                  <div className="flex justify-between">
                    <span className="text-gray-600">{t('binarization.blackPixels')}:</span>
                    <span className="font-mono">{binarization.binary.filter(v => v === 255).length.toLocaleString()}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-gray-600">{t('binarization.whitePixels')}:</span>
          ...
          <div className="p-2 bg-blue-50 rounded text-xs">
            <div className="font-medium mb-1">{t('binarization.sauvolaTitle')}</div>
            <div className="space-y-0.5 text-gray-700">
              <div>• {t('binarization.sauvolaStep1')}</div>
              <div>• {t('binarization.sauvolaStep2')}</div>
              <div>• {t('binarization.sauvolaStep3')}</div>
              <div>• {t('binarization.sauvolaStep4')}</div>
            </div>
          </div>
        </div>
      ) : (
        <div className="space-y-4">
          ...
          <div className="p-8 bg-gray-50 rounded text-center">
            ...
            <div className="text-gray-500 text-sm">{t('binarization.noImageDescription')}</div>
          </div>
        </div>
      )}
    </div>
