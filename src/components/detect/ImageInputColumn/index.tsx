    ...
    <div className="step-column">
      ...
      <div className="space-y-4">
        <p className="text-sm text-gray-600">
          {t('QR 코드 이미지를 업로드하거나 카메라로 캐팁합니다', 'Upload a QR code image or capture using camera')}
        </p>

        {/* 입력 모드 선택 버튼 */}
        <div className="p-3 bg-gray-50 rounded">
          <div className="text-xs font-medium mb-2">{t('입력 방법 선택', 'Select Input Method')}</div>
              ...
              <div className="p-3 bg-gray-50 rounded">
                <div className="flex items-center justify-center h-64">
                  <div className="text-center">
                    ...
                    <p className="text-sm text-gray-600">{t('가상 카메라 로딩 중...', 'Loading virtual camera...')}</p>
                  </div>
                </div>
              </div>
            }
        ...
        {imageUrl && inputMode === 'file' && (
          <div className="space-y-3">
            <div className="p-3 bg-gray-50 rounded">
              <div className="text-xs font-medium mb-2">{t('업로드된 이미지', 'Uploaded Image')}</div>
              <img src={imageUrl} alt="Input QR Code" className="w-full h-auto rounded border border-gray-200" />
              {imageProcessing && (
                <div className="mt-2 text-xs text-gray-500">
                  {t('크기', 'Size')}: {imageProcessing.width} × {imageProcessing.height}px
                </div>
              )}
            </div>
          </div>
        ...
        {(inputMode === 'camera' || inputMode === 'virtual') && imageProcessing && (
          <div className="p-3 bg-gray-50 rounded">
            <div className="text-xs font-medium mb-2">{t('처리 상태', 'Processing Status')}</div>
            <div className="space-y-1 text-xs">
              <div className="flex justify-between">
                <span className="text-gray-600">{t('프레임 크기:', 'Frame Size:')}</span>
                <span className="font-mono">{imageProcessing.width} × {imageProcessing.height}px</span>
              </div>
              <div className="flex justify-between">
                <span className="text-gray-600">{t('상태:', 'Status:')}</span>
                <span className="font-mono">{isProcessing ? t('분석 중...', 'Analyzing...') : t('실시간 모니터링 중', 'Live monitoring')}</span>
              </div>
            </div>
          </div>
        )}
        ...
        <div className="p-2 bg-blue-50 rounded text-xs">
          <div className="font-medium mb-1">{t('입력 방법', 'Input Methods')}</div>
          <div className="space-y-0.5 text-gray-700">
            <div>• {t('파일 업로드: 드래그 앤 드롭 또는 클릭하여 선택', 'File Upload: Drag and drop or click to select')}</div>
            <div>• {t('카메라: 수동/자동 모드로 실시간 캐팁', 'Camera: Manual/auto mode for real-time capture')}</div>
            <div>• {t('가상 카메라: 3D QR 코드를 다양한 각도에서 테스트', 'Virtual Camera: Test 3D QR codes from various angles')}</div>
          </div>
        </div>
      </div>
    </div>
