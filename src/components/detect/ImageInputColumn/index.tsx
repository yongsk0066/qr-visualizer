...
import { VirtualCameraInput } from './VirtualCameraInput';
...
export function ImageInputColumn({ 
  ...
  // encodedQRMatrix가 처음 생성될 때만 virtual 모드로 전환
  useEffect(() => {
    if (encodedQRMatrix && !hasAutoSwitched) {
      setInputMode('virtual');
      setHasAutoSwitched(true);
    }
  }, [encodedQRMatrix, hasAutoSwitched]);
  ...
  return (
    <div className="step-column">
      ...
      {/* 파일 업로드 UI */}
      {inputMode === 'file' && (
        <FileInput onImageSelect={setImageUrl} />
      )}
