...
export function QRCodeMesh({ matrix, damageSpots }: QRCodeMeshProps) {
  ...
  const whiteMaterial = useMemo(() => new THREE.MeshBasicMaterial({ color: 'white' }), []);
  ...
  useEffect(() => {
    ...
    damageSpots.forEach((spot) => {
      ...
      const matrixRow = Math.floor(canvasY / scale);
    ...
    if (meshRef.current) {
      const qrMaterial = new THREE.MeshBasicMaterial({ map: texture });
      // BoxGeometry face order: +x, -x, +y, -y, +z (front), -z (back)
      // Apply QR texture only to front face (+z), white for all other faces
      meshRef.current.material = [
        whiteMaterial, // right (+x)
        whiteMaterial, // left (-x)
        whiteMaterial, // top (+y)
        whiteMaterial, // bottom (-y)
        qrMaterial,    // front (+z) - QR code visible here
        whiteMaterial, // back (-z)
      ];
    }
  }, [matrix, damageSpots, scale, quietZone, whiteMaterial]);

  return (
    <mesh ref={meshRef} position={[0, 0, 0]}>
      <boxGeometry args={[...QR_BOX_SIZE]} />
    </mesh>
  );
}