...
export function countTimingPatternModules(
  ...
  const horizontalStart = Math.round(topLeftPattern.center.x + topLeftPattern.size / 2 + moduleSize);
  const horizontalEnd = Math.round(topRightPattern.center.x - topRightPattern.size / 2 - moduleSize);
  
  // 수직 타이밍 패턴 분석 (TL에서 BL까지)
  const verticalStart = Math.round(topLeftPattern.center.y + topLeftPattern.size / 2 + moduleSize);
  const verticalEnd = Math.round(bottomLeftPattern.center.y - bottomLeftPattern.size / 2 - moduleSize);
  ...
  console.log('Timing pattern analysis:', {
    ...
    horizontalStart,
    horizontalEnd,
    horizontalDistance: horizontalEnd - horizontalStart,
    verticalStart,
    verticalEnd,
    verticalDistance: verticalEnd - verticalStart,
    finderCenters: {
      topLeft: topLeftPattern.center,
      topRight: topRightPattern.center,
      bottomLeft: bottomLeftPattern.center
    }
  });
  ...
  console.log('Timing pattern modules:', {
  ...
  console.log('Total modules from timing pattern:', {
    totalModulesH,
    totalModulesV,
    totalModules,
    closestValidSize: closestSize
  });
...
function scanTimingPattern(
  ...
  // 각 런의 길이를 모듈 크기로 나누어 모듈 수 계산
  const modulesPerRun = avgRunLength / moduleSize;
  ...
  // 타이밍 패턴의 시작과 끝이 무엇인지 확인
  const startsWithBlack = runs[0].color === 0;
  const endsWithBlack = runs[runs.length - 1].color === 0;
  
  console.log(`${horizontal ? 'Horizontal' : 'Vertical'} timing pattern:`, {
    ...
    startsWithBlack,
    endsWithBlack,
    pattern: runs.slice(0, 10).map(r => `${r.color ? 'W' : 'B'}:${r.length}`).join(' ') + (runs.length > 10 ? '...' : '')
  });
